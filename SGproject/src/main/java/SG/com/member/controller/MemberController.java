...
public class MemberController 
{
   ...
   public String memDeleteAction(HttpServletResponse response, HttpServletRequest request, HttpSession session, Model model, CommandMap commandMap) throws Exception 
   {
      ...
      Map<String,Object> onoff = memberService.myinfoDetail(commandMap.getMap());
      int OnOff = Integer.parseInt(onoff.get("MEMBER_ONOFF").toString());
      
      
      if(OnOff == 1)
      {
    	  model.addAttribute("MEMBER_ONOFF", OnOff);
    	  session.invalidate();
      }
